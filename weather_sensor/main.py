...
def main():
    isconnected = wifi.connect()
    client = mqtt.connect()
    while True:
        if not client:
            client = mqtt.connect(1)
        ...
        #writing to file
        if isconnected and client:
            print(timestring + ' Sending to mqtt broker')
            client.publish(mqtt.temp, str.encode('{}'.format(temperature)))
            client.publish(mqtt.pres, str.encode('{}'.format(pressurehpa)))
            client.publish(mqtt.humid, str.encode('{}'.format(humidity)))

        # waits for 20 seconds and clears to BLACK
        sleep(20)
